...
<body bgcolor="${bodyback_c }">
		...
		<td align="center">
			<c:if test="${count>0 }">
				...
				<c:if test="${startPage > 10 }">
					<a href="/socialfood/board/list.do?pageNum=${startPage-10 }">[이전]</a>
				</c:if>
				<c:forEach var="i" begin="${startPage }" end="${endPage }">
					<a href="/socialfood/board/list.do?pageNum=${i }">${i }</a>
				</c:forEach>
				<c:if test="${endPage < pageCount }">
					<a href="/socialfood/board/list.do?pageNum=${startPage + 10 }">[다음]</a>
				</c:if>
			</c:if>
		</td>
	</tr>
	...
	<form action="/socialfood/board/search.do?pageNum=1&search=${search}&val=${val}" name="findForm">
		<table width="800" border="0" cellpadding="0" cellspacing="0" align="center">
			...
			<input type="button" onclick="document.location.href='/socialfood/board/writeForm.do?area=${area}'" value="글쓰기">	
			</td>
				<td align="right">
					<select name="search">
