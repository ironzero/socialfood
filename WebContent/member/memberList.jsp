...
<body>
<div align="center"><b>회원 리스트(전체 회원 수 : ${count})</b></div><br />
	...
	<c:if test="${count > 0}">
		...
		<c:if test="${startPage > 10}">
			<a href="/socialfood/memberList.do?pageNum=${startPage - 10}">[이전]</a>
		</c:if>
		<c:forEach var="i" begin="${startPage}" end="${endPage}">
			<a href="/socialfood/memberList.do?pageNum=${i}">[${i}]</a>
		</c:forEach>
		<c:if test="${endPage < pageCount }">
			<a href="/socialfood/memberList.do?pageNum=${startPage + 10}">[다음]</a>
		</c:if>
	</c:if>
	<br />
	</center>
		...
		<c:forEach var="member" items="${memberList}">
				...
				<tr class="ac">
					<td align="right" colspan="7">
						...
						<input type="button" value="작성 글 보기" onclick="myBoardList('${member.id}')"/>
						<input type="button" value="강제 탈퇴" onclick="deleteMember('${member.id}')"/>
					</td>
				</tr>
			</c:forEach>	
		</table>
