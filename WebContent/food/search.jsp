...
<body bgcolor="${bodyback_c }">
			...
			<c:forEach var="article" items="${articleList}">
				<tr height="30">
					...
					<td width="250">
						...
						[${article.category}] <a href="/socialfood/food/content.do?idx=${article.idx }&pageNum=${currentPage}">
							 ${article.title } </a>
	...
	<c:if test="${count>0 }">
		...
		<c:if test="${startPage > 10 }">
			<a href="/socialfood/food/search.do?pageNum=${startPage-10 }">[이전]</a>
		</c:if>
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			<a href="/socialfood/food/search.do?pageNum=${i }">${i }</a>
		</c:forEach>
		<c:if test="${endPage < pageCount }">
			<a href="/socialfood/food/search.do?pageNum=${startPage + 10 }">[다음]</a>
		</c:if>
	</c:if>
	<br />
	<form action="/socialfood/food/search.do?pageNum=1&search=${search}&val=${val}" name="findForm">
		<table width="800" border="0" cellpadding="0" cellspacing="0">
			<tr>
			<td align="left">
			<input type="button" onclick="document.location.href='/socialfood/food/writeForm.do'" value="글쓰기">	
			</td>
				<td align="right">
					<select name="search">
